...
async def _(event):
    ...
    if not event.is_reply:
        await event.edit("**Rispondi ad una foto per creare lo Sticker.**")
        return
    ...
    async with borg.conversation("@Stickers") as bot_conv:
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            await event.edit("**📦Creando un nuovo Pack!**")
...
async def _(event):
    if event.fwd_from:
        return
    if not event.is_reply:
        await event.edit("Rispondi ad uno Sticker per avere le informazioni del Pack.")
        return
    ...
    await event.edit(f"**💬Titolo Sticker:** `{get_stickerset.set.title}\n`"
                     f"**🔖Soprannome Sticker:** `{get_stickerset.set.short_name}`\n"
                     f"**💼Ufficiale:** `{get_stickerset.set.official}`\n"
                     f"**🗄Archiviato:** `{get_stickerset.set.archived}`\n"
                     f"**🗂Stickers nel Pack:** `{len(get_stickerset.packs)}`\n"
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if not sticker_attrib.stickerset:
            await event.reply("**Questo sticker non farte di un Pack.**")
            return
        is_a_s = is_it_animated_sticker(reply_message)
        ...
        await event.edit(f"Scaricando {sticker_set.set.count} sticker in .{Config.TMP_DOWNLOAD_DIRECTORY}{sticker_set.set.short_name}...")
        ...
        while 1:
            ...
            if not pending_tasks:
                break
        await event.edit("Scaricando nel mio server Locale.")
        # https://gist.github.com/udf/e4e3dbb2e831c8b580d8fddd312714f7
        ...
        try:
            ...
            pass
        await event.edit("Lavoro Completato.")
